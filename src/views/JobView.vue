...
const deleteJob = async () => {
  try {
    ...
    if (confirm) {
      ...
      router.push('/jobs')
      useToast().success('Job successfuly deleted')
    }
  } catch {
    useToast().error('Error in deleting job')
  }
}

onMounted(async () => {
  try {
    const jobData = await axios.get('/api/jobs/' + id)
...
<template>
  <section class="bg-green-50 md:px-16">
    <div class="containerm-auto py-10" v-if="job">
    ...
    <div v-else class="flex flex-col gap-10 items-center py-16">
      <h2 class="text-xl font-bold text-gray-600">Error fetching the job :(</h2>

      <RouterLink to="/jobs" class="bg-green-600 px-3 py-1 rounded-md text-white"
        >Go back</RouterLink
      >
    </div>
  </section>
