...
async function editJob() {
  try {
    ...
    const response = await axios.patch(`/api/jobs/${id}`, updatedJob)
    router.push(`/jobs/${response.data.id}`)
    useToast().success('Job successfuly edited')
  } catch (error) {
    console.error(error.message)
    useToast().error('Error occurred when updating the job')
  }
}
</script>
        ...
        <form @submit.prevent="editJob">
          ...
          <div class="mb-4">
            ...
            <input
              ...
              placeholder="eg. Junior Software Developer"
              required
            />
          </div>
          <div class="mb-4">
